...
def init_db():
    with _connect() as conn:
        ...
        """)
        try:
            conn.execute("ALTER TABLE users ADD COLUMN own_model TEXT")
        except Exception:
            pass
        conn.commit()
...
def save_user_api_key(user_id: int, encrypted_key: str, base_url: str, model: str = ""):
    with _connect() as conn:
        conn.execute(
            "UPDATE users SET own_api_key_enc = ?, own_base_url = ?, own_model = ? WHERE id = ?",
            (encrypted_key, base_url, model or None, user_id),
        )
        conn.commit()
...
def clear_user_api_key(user_id: int):
    with _connect() as conn:
        conn.execute(
            "UPDATE users SET own_api_key_enc = NULL, own_base_url = NULL, own_model = NULL WHERE id = ?",
            (user_id,),
        )
        conn.commit()
